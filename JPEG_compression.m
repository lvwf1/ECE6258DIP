...
XBitRate = length(XBitStream)/prod(size(X)); %Calculate the bitRate 
...
X_DCT = blockproc(X,[8 8],fun);% use blockproc and dct2 to calculate blockwise DCT.
...
% EDIT THIS PART
qfun = @(block_struct) round(block_struct.data./Q/c);
XQ = blockproc(X_DCT,[8 8],qfun);  %use blockproc to quantize the X_DCT in blocks  
...
% EDIT THIS PART
XQBitRate = length(XQBitStream)/prod(size(XQ)); %calculate the bit rate
...
dqfun = @(block_struct) block_struct.data.*Q;
XdeQ_reconstructed = blockproc(XQ_reconstructed,[8 8],dqfun);;

%% STEP 13: Inverse DCT
%EDIT THIS PART
ifun = @(block_struct) idct2(block_struct.data);
X_reconstructed = blockproc(XdeQ_reconstructed,[8 8],ifun);% use blockproc and idct2 to calculate blockwise iDCT of XdeQ_reconstructed. 
...
MSE = immse(X_reconstructed,X); %Calculate MSE 
PSNR = psnr(X_reconstructed,X,255); %Calculate PSNR. Use MATLAB function psnr and set PEAKVAL to 255
