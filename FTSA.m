...
switch popupmenu1value
   case 'JPEG Compression'
       ...
       temp = inputdlg(prompt,dlg_title,[1 50]);
       if isempty(temp)
           % user pressed cancel
           return
       end
   ...
   case 'JPEG Compression without ZigZag'
       ...
       if(~isempty(bs)&&~isempty(q))
           ...
           elseif q<0 && q>100
               ...
               fun = @(block_struct) dct2(block_struct.data);
               img_DCT = blockproc(double(img),[bs bs],fun);
    ...
    case 'Filter Denoise'
       ...
       if(~isempty(answer1))
           ...
           elseif strcmp(answer1, 'gaussian')
               prompt = {'Enter hsize for a gaussian filter:', 'Enter sigmafor a gaussian filter:'};
               dlg_title = 'Faussian Filter Variables';
               temp = inputdlg(prompt,dlg_title,[1 50]);
               [hsize,sigma] = temp{:};
               hsize = str2double(hsize);
